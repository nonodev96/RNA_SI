...
x, y = make_regression(
    n_samples=100, n_features=1, n_informative=1, noise=20, random_state=2017
)
x = x.flatten()
...
theta = history[-1]
print(
    "Gradient Descent: {:.2f}, {:.2f} {:d}".format(theta[0], theta[1], iters)
)
...
zs = np.array(
    [error(xaug, y, theta) for theta in zip(np.ravel(M), np.ravel(B))]
)
...
ax.plot(
    [t[0] for t in history],
    [t[1] for t in history],
    cost,
    markerfacecolor="r",
    markeredgecolor="r",
    marker=".",
