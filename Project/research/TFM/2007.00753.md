...
In, [56], introduced a one step adversarial attack framework. The attack image, x′, is obtained by a simple additive disturb:
...
3.1.8 Universal Adversarial Perturbations Different from the previous methods, the universal adversarial perturbation (UAP), proposed in [66], search for a single perturbation capable of fooling all samples from the training dataset. The perturbations, independent of the input, are also restricted to not be detected by humans. The perturbations are constructed based on:
