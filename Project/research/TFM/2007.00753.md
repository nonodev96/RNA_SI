...
$$ \underset{\delta\in\Delta}{\text{max}} ~\mathcal{L}(f(x+\delta), y) $$
...
$$
\underset{{\lVert \delta_{ut} \rVert}_{p} \leq \epsilon}{\text{max}} ~ \mathcal{L}(f(x+\delta), y)
$$

and in the the targeted setting, $\delta_{tg}$, from:

$$
\underset{{\lVert \delta_{tg} \rVert}_{p} \leq \epsilon}{\text{max}} ~ (\mathcal{L}(f(x+\delta), y) - \mathcal{L}(f(x+\delta), y^{\prime}))
$$
