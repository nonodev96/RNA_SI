...
def KNN_detection(Train, labelsTrain, k=10):
	...
	nbrs = NearestNeighbors(n_neighbors=k, algorithm='auto').fit(Train) #ball_tree
	_, Neighbors = nbrs.kneighbors(Train)  # Neighbors contains indices of samples
	NeighborsLabels = labelsTrain[Neighbors]
  
	Labels = stats.mode(NeighborsLabels, axis=1)[0].squeeze()
			
	attacks = labelsTrain != Labels
	print(attacks.sum())
...
def main(N0, N1, N2, N3, N4, classifier='SVM'):
	...
	# output: A, indicators of conjectured malicious samples, 0-normal, 1-malicious
	A = KNN_detection(features_train, trainset.targets, k=10)

	# 4. performance of the anomaly detector
	# 1)True positive rate(tpr): # turly detected malicious samples/ total # malicious samples
