...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figures/chapter02/machine-learning.drawio.png}
...
Dentro de la inteligencia artificial encontramos \gls{AI} el aprendizaje automático \gls{ML} y dentro el aprendijaze profundo \gls{DL}
...
\subsubsection{Ataques en \textit{deep learning}}

Existen cuatro grandes formas de clasificar los ataques en \gls{ML}

% 
\textbf{Evasión}: busca explotar las vulnerabilidades de la red neuronal para inducir errores en la capacidad de clasificar o de predecir. Se envia información con perturbaciones minimas que alteren notablemente la respuesta del modelo.
\begin{itemize}
    \item Caja blanca: se conoce todo sobre el modelo, arquitectura, pesos, umbrales, formato de entreada de datos y salida, etc. \cite{learning-machine-learning-part-3-attacking}
    \item Caja negra: se desconoce el modelo, arquitectura, pesos, umbrales, pero se conoce el formato de entrada de datos y la respuesta del modelo. \cite{learning-machine-learning-part-3-attacking}
\end{itemize}

% 
\textbf{Envenenamiento}: los atacantes intentan manipular los datos de entrenamiento con el objetivo de influir en el resultado del aprendijaze del modelo, esto pueden hacerlo desde distintas etapas.

$$\sum_{r a n d}(x)=\left\{c l i p\left(x+\delta\right)|\delta\in[-5,5]^{H\times W\times3}\right\}$$
\begin{itemize}
    \item Ataques de cambio de etiquetas \textit{Label Flipping Attacks}
    \item Ataque de envenenamiento de datos de etiquetado limpio \textit{Clean Label Data Poisoning Attack}
    \item Ataque de puerta trasera \textit{Backdoor Attack}
\end{itemize}

% 
\textbf{Extracción}: se busca extraer un funcionamiento interno equivalente de la red neuronal, se puede considerar un ataque de ingeniería inversa.
...
\begin{itemize}
    \item Atributos
    \item Pertenencia
    \item Inversión de modelos
    \item Reconstrucción
\end{itemize}


\subsubsection{Defensas contra los ataques en \textit{deep learning}}


\subsubsection{Metricas en los ataques adversariales}

\subsubsection{Red team y blue team en Rede}
...
Se puede ser categorizar la seguridad de los modelos con el modelado \gls{STRIDE} de microsoft\footnote{Modelado de amenazas de microsoft \href{https://learn.microsoft.com/es-es/azure/security/develop/threat-modeling-tool-threats}{STRIDE}} o con la matriz \gls{MITRE}.
