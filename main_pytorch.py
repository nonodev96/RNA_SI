...
def generate_x_target():
    ...
    image = Image.open("./data/x-target-img/bad-apple.png")
    ...
    print(np.save("./data/x-target/bad-apple-1x28x28-gris.npy", img_tensor.numpy()))
    # torchvision.utils.save_image(img_tensor.to("cpu", torch.uint8).numpy(), f"./results/images/x-target-torchvision.png")

    # Se espera que el tensor tenga un formato (height, width, channels), 
    # pero el tensor que estás pasando tiene el formato (channels, height, width), típico de PyTorch.
